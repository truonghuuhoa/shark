...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
		...
		<tbody>
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					echo '<td><h5 class="my-2"><b><span class="text-dark woocommerce-grouped-product-list-item__' . esc_attr( $column_id ) . '">' . apply_filters( 'woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product_child ) . '</span></b></h5></td>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
		...
		<button type="submit" class="btn btn-primary waves-effect waves-light mt-2 me-1 button alt"><i class="bx bx-cart me-2"></i><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
