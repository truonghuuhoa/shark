...
class FCMS_WooThumbs {
	/**
	 * Version
	 *
	 * @var string
	 */
	public $version = '4.8.7';
	 ...
	 *
	 * @var FCMS_WooThumbs_Transition_Settings
	 */
	...
	function __construct() {
		...
		if ( ! FCMS_WooThumbs_Core_Helpers::is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			return;
		}
	...
	private function define_constants() {
		$this->define( 'FCMS_WOOTHUMBS_PATH', $this->plugin_path );
		$this->define( 'FCMS_WOOTHUMBS_URL', $this->plugin_url );
		$this->define( 'FCMS_WOOTHUMBS_INC_PATH', $this->plugin_path . 'inc/' );
		$this->define( 'FCMS_WOOTHUMBS_VENDOR_PATH', FCMS_WOOTHUMBS_INC_PATH . 'vendor/' );
		$this->define( 'FCMS_WOOTHUMBS_BASENAME', plugin_basename( __FILE__ ) );
		$this->define( 'FCMS_WOOTHUMBS_IS_ENVATO', false );
	}
	...
	private function init_autoloader() {
		require_once( FCMS_WOOTHUMBS_INC_PATH . 'class-core-autoloader.php' );

		FCMS_WooThumbs_Core_Autoloader::run( array(
			'prefix'   => 'FCMS_WooThumbs_',
			'inc_path' => FCMS_WOOTHUMBS_INC_PATH,
		) );
	}
	...
	private function load_classes() {
		...
		FCMS_WooThumbs_Core_Settings::run( array(
			'vendor_path'   => FCMS_WOOTHUMBS_VENDOR_PATH,
			'title'         => $this->name,
			'version'       => $this->version,
			'menu_title'    => 'WooThumbs',
			'settings_path' => FCMS_WOOTHUMBS_INC_PATH . 'admin/settings.php',
		...
		FCMS_WooThumbs_Settings::run();
		FCMS_WooThumbs_Update::run();
		FCMS_WooThumbs_Template_Hooks::run();
		FCMS_WooThumbs_Shortcodes::run();
		FCMS_WooThumbs_Media::run();
		FCMS_WooThumbs_Images::run();
		FCMS_WooThumbs_Product_Variation::run();
		FCMS_WooThumbs_Import::run();

		FCMS_WooThumbs_Compat_Yith_Badge::run();
		FCMS_WooThumbs_Compat_Astra::run();
		FCMS_WooThumbs_Compat_Elementor::run();
		FCMS_WooThumbs_Compat_Divi::run();
		FCMS_WooThumbs_Compat_Flatsome::run();
		FCMS_WooThumbs_Compat_WP_All_import::run();
	}
	...
	public function set_settings() {
		$this->settings = FCMS_WooThumbs_Core_Settings::$settings;
	}
	...
	function plugins_loaded_hook() {
		load_plugin_textdomain( 'fcms-woothumbs', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );

		if ( apply_filters( 'fcms_woothumbs_is_admin', is_admin() ) ) {
			...
			add_action( 'admin_menu_fcms_woothumbs', array( $this, 'bulk_edit_page' ), 10 );
			add_action( 'admin_init', array( $this, 'media_columns' ) );

			add_action( 'wp_ajax_fcms-woothumbs_bulk_save', array( $this, 'bulk_save' ) );
			...
			add_action( 'wp_ajax_fcms_woothumbs_get_variation', array( $this, 'ajax_get_variation' ) );
			add_action( 'wp_ajax_nopriv_fcms_woothumbs_get_variation', array( $this, 'ajax_get_variation' ) );
		} else {
			...
			add_action( 'fcms_woothumbs_after_images', array( $this, 'add_yith_wishlist_icon' ) );
	...
	public function is_enabled( $post_id = null ) {
		...
		$enabled_filtered = apply_filters( 'fcms_woothumbs_is_enabled', null, $post_id );
	...
	public function bulk_edit_page() {
		add_submenu_page( 'woocommerce', __( 'Bulk Edit Variation Images', 'fcms-woothumbs' ), sprintf( '<span class="fs-submenu-item fs-sub woothumbs">%s</span>', __( 'Bulk Edit', 'fcms-woothumbs' ) ), 'manage_woocommerce', $this->bulk_edit_slug, array( $this, 'bulk_edit_page_display' ) );
	}
	...
	public function bulk_edit_page_display() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'You do not have sufficient permissions to access this page.', 'fcms-woothumbs' ) );
		}
	...
	public function product_tab( $tabs ) {
		...
		echo '<li class="' . $this->slug . '_options_tab"><a href="#' . $this->slug . '_options"><span>' . __( 'WooThumbs', 'fcms-woothumbs' ) . '</span></a></li>';
	}
	...
	public function product_tab_fields() {
		...
		?>
		<div id="fcms-woothumbs_options" class="panel woocommerce_options_panel">
			<?php
			woocommerce_wp_checkbox(
				array(
					'id'    => 'disable_woothumbs',
					'label' => __( 'Disable WooThumbs?', 'fcms-woothumbs' ),
				)
			);

			woocommerce_wp_text_input( array(
				'id'          => 'fcms-woothumbs-video-url',
				'name'        => 'fcms_woothumbs[video_url]',
				'label'       => __( 'Video URL', 'fcms-woothumbs' ),
				'desc_tip'    => true,
				'description' => __( 'Enter a video URL to display in the lightbox only ( a video play icon will be placed over the gallery images ). Most video hosting services are supported.', 'fcms-woothumbs' ),
				'value'       => esc_attr( FCMS_WooThumbs_Product::get_setting( $post->ID, 'video_url' ) ),
			) );

			woocommerce_wp_checkbox( array(
				'id'            => 'fcms-woothumbs-maintain-slide-index',
				'name'          => 'fcms_woothumbs[maintain_slide_index]',
				'label'         => __( 'Maintain Slide Index?', 'fcms-woothumbs' ),
				'desc_tip'      => true,
				'description'   => __( 'When checked, the slide index will be maintained upon changing images. For example, if you\'re viewing the second image in the slider, and change variation, the second image will be selected from the new images.', 'fcms-woothumbs' ),
				'value'         => ! is_null( FCMS_WooThumbs_Product::get_setting( $post->ID, 'maintain_slide_index' ) ) ? 'yes' : 'no',
				'wrapper_class' => 'show_if_variable',
			) );

			woocommerce_wp_checkbox( array(
				'id'            => 'fcms-woothumbs-maintain-product-gallery',
				'name'          => 'fcms_woothumbs[maintain_product_gallery]',
				'label'         => __( 'Maintain Gallery?', 'fcms-woothumbs' ),
				'desc_tip'      => true,
				'description'   => __( 'When checked, the main product gallery will also be used for variations.', 'fcms-woothumbs' ),
				'value'         => ! is_null( FCMS_WooThumbs_Product::get_setting( $post->ID, 'maintain_product_gallery' ) ) ? 'yes' : 'no',
				'wrapper_class' => 'show_if_variable',
			) );
			?>
		</div>
	...
	public function save_product_fields( $post_id ) {
		...
		if ( isset( $_POST['fcms_woothumbs'] ) ) {
			if ( isset( $_POST['fcms_woothumbs']['video_url'] ) ) {
				$product_settings['video_url'] = sanitize_text_field( $_POST['fcms_woothumbs']['video_url'] );
			}

			if ( isset( $_POST['fcms_woothumbs']['maintain_slide_index'] ) ) {
				$product_settings['maintain_slide_index'] = 'yes';
			}

			if ( isset( $_POST['fcms_woothumbs']['maintain_product_gallery'] ) ) {
				$product_settings['maintain_product_gallery'] = 'yes';
			}

			update_post_meta( $post_id, '_fcms_woothumbs', $product_settings );
		}
	...
	public function delete_transients( $force = false, $product_id = false ) {
		if ( isset( $_POST['fcms-woothumbs-delete-image-cache'] ) || $force === true ) {
			global $wpdb;
	...
	function bulk_save() {
		...
		switch ( $return['result'] ) {
			case 'invalid-format':
				$return['message'] = __( 'Please use only numbers and commas.', 'fcms-woothumbs' );
				break;
			case 'failed':
				$return['message'] = __( 'Sorry, an error occurred. Please try again.', 'fcms-woothumbs' );
				break;
		}
	...
	public function admin_scripts() {
		...
		if (
			...
			( $get_page && ( 'fcms-woothumbs-settings-account' === $get_page || 'fcms-woothumbs-settings' === $get_page ) ) ||
			'upload.php' === $pagenow ||
			...
			wp_enqueue_style( 'fcms_woothumbs_admin_css', plugins_url( 'assets/admin/css/admin-styles.css', __FILE__ ), false, '2.0.1' );
			...
			wp_localize_script( $this->slug, 'fcms_woothumbs_vars', $vars );
		}
	}
	...
	public function register_scripts_and_styles() {
		...
		if ( apply_filters( 'fcms_woothumbs_load_assets', false ) || $jckqv || ( ( function_exists( 'is_product' ) && is_product() ) || has_shortcode( $post->post_content, 'product_page' ) || has_shortcode( $post->post_content, 'woothumbs-gallery' ) ) && apply_filters( 'woothumbs_enabled', $this->is_enabled() ) || is_woocommerce() || is_product_category() ) {
			// Plugins/Libs
			...
			$vars = array(
				...
				'text'         => array(
					'fullscreen' => __( 'Fullscreen', 'fcms-woothumbs' ),
					'video'      => __( 'Play Video', 'fcms-woothumbs' ),
				),
				'is_rtl'       => is_rtl(),
			);

			wp_localize_script( $this->slug . '-script', 'fcms_woothumbs_vars', $vars );
	...
	public function maybe_enable_fullscreen() {
		...
		return apply_filters( 'fcms_woothumbs_enable_fullscreen', (
			$this->settings['fullscreen_general_enable'] ||
			! is_null( FCMS_WooThumbs_Product::get_setting( $post->ID, 'video_url' ) ) ||
			$jckqv
		) );
	}
		...
		if ( false === ( $default_variation_id = get_transient( $transient_name ) ) ) {
			if ( $product->get_type() == 'variable' ) {
				...
				if ( ! empty( $default_attributes ) ) {
					$variation_id = FCMS_WooThumbs_Product::find_matching_product_variation( $product, $default_attributes );
	...
	public function get_selected_variation( $current_id ) {
		...
		if ( $product->get_type() == 'variable' ) {
			...
			$variation = FCMS_WooThumbs_Product::find_matching_product_variation( $product, $selected_atts );
	...
	public function get_transient_name( $id, $type ) {
		...
		return apply_filters( 'fcms_woothumbs_transient_name', $transient_name, $type, $id );
	}
	...
	public function get_all_image_ids( $id ) {
		...
		if ( false === ( $image_ids = get_transient( $transient_name ) ) ) {
			$image_ids     = array();
			$product       = FCMS_WooThumbs_Product::get_product( $id );
			...
			// Add gallery images.
			$image_ids = array_merge( $image_ids, FCMS_WooThumbs_Product::get_gallery_image_ids( $product ) );

			// Set placeholder or parent images if no images present.
			if ( empty( $image_ids ) ) {
				$parent_id = FCMS_WooThumbs_Product::get_parent_id( $product );
		...
		return apply_filters( 'fcms_woothumbs_all_image_ids', $image_ids, $id );
	}
	...
	public function get_all_image_sizes( $product_id ) {
		...
		if ( ! empty( $image_ids ) ) {
			foreach ( $image_ids as $image_id ):
				...
				if ( false === $image_data ) {
					$image_data = FCMS_WooThumbs_Images::get_attachment_props( $image_id, $product_id );
					$image_data = apply_filters( 'fcms_woothumbs_single_image_data', $image_data, $image_id, $product_id );
		...
		return apply_filters( 'fcms_woothumbs_all_images_data', $images, $product_id );
	}
	...
	public static function get_image_loop_data( $image, $index ) {
		...
		$image_data = array(
			...
			'data-fcms-woothumbs-src' => $index == 0 ? false : $image['src'],
			'class'                     => 'fcms-woothumbs-images__image no-lazyload skip-lazy',
	...
	public function add_yith_wishlist_icon() {
		if ( in_array( 'yith-woocommerce-wishlist/init.php', $this->active_plugins ) || in_array( 'yith-woocommerce-wishlist-premium/init.php', $this->active_plugins ) ) {
			...
			<div class="fcms-woothumbs-wishlist-buttons <?php if ( $added ) {
				echo "fcms-woothumbs-wishlist-buttons--added";
			} ?>">

				<a class="fcms-woothumbs-wishlist-buttons__browse" href="<?php echo $wishlist_url; ?>" data-fcms-woothumbs-tooltip="<?php _e( 'Browse Wishlist', 'fcms-woothumbs' ); ?>"><i class="fcms-woothumbs-icon fcms-woothumbs-icon-heart"></i></a>

				<a href="<?php echo esc_url( add_query_arg( 'add_to_wishlist', $product->get_id() ) ) ?>" rel="nofollow" data-product-id="<?php echo $product->get_id(); ?>" data-product-type="<?php echo $product->get_type(); ?>" class="fcms-woothumbs-wishlist-buttons__add add_to_wishlist" data-fcms-woothumbs-tooltip="<?php _e( 'Add to Wishlist', 'fcms-woothumbs' ); ?>"><i class="fcms-woothumbs-icon fcms-woothumbs-icon-heart"></i></a>
...
$fcms_woothumbs_class = new FCMS_WooThumbs();
