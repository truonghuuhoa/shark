(function( $, document ) {
	...
	function setup_bulk_save_buttons() {
		$( '.saveVariationImages' ).on( 'click', function() {
			...
			var btn = $( this ),
				...
				ajaxargs = {
					action: fcms_woothumbs_vars.slug + '_bulk_save',
					nonce: fcms_woothumbs_vars.nonce,
					varid: varid,
					images: images
				};
			...
			$.post( fcms_woothumbs_vars.ajaxurl, ajaxargs, function( data ) {
	...
	function setup_media_upload_for_attachment_page( $button ) {
		$( document.body ).on( 'click', '.fcms-wt-upload-media', function( event ) {
			...
			if ( wp.media.frames.fcms_woothumbs_media ) {
				wp.media.frames.fcms_woothumbs_media.media_thumbnail_id = media_thumbnail_id;
				wp.media.frames.fcms_woothumbs_media.open();
				return;
			}

			// Create the media frame.
			wp.media.frames.fcms_woothumbs_media = wp.media( {
				// Set the title of the modal.
				title: 'Select MP4',
				button: {
					text: 'Attach MP4'
			...
			// When an image is selected, run a callback.
			wp.media.frames.fcms_woothumbs_media.on( 'select', function() {
			
				var selected_media = wp.media.frames.fcms_woothumbs_media.state().get( 'selection' ),
					$media_field = $( '#attachments-' + wp.media.frames.fcms_woothumbs_media.media_thumbnail_id + '-fcms_woothumbs_media' );
	...
	function setup_media_upload_for_product_and_library_page( ) {
		$( document.body ).on( 'click', '.fcms-wt-upload-media', function( event ) {
			...
			// wp.media.frames.fcms_woothumbs_media.on('all', function(e) { console.log( 'attach mp4', e); });
			...
			wp.media.frames.fcms_woothumbs_media.on( 'select', function() {
				...
				if( wp.media.frames.original.state().get('selection') ) { // returns false in media library page.
					wp.media.frames.original.state().get('selection').add( wp.media.attachment( wp.media.frames.fcms_woothumbs_media.media_thumbnail_id ) );
				}
