...
/**
 * FCMS_WooThumbs_Product.
 *
 * @class    FCMS_WooThumbs_Product
 * @version  1.0.0
...
class FCMS_WooThumbs_Product {
	...
	public static function find_matching_product_variation( $product, $attributes ) {
		...
		$transient_name    = sprintf( 'fcms-woothumbs_matching_variation_%d_%s', $product->get_id(), $attributes_md5 );
	...
	public static function get_settings( $product_id ) {
		...
		if ( empty( $product_settings[ $product_id ] ) ) {
			$product_settings[ $product_id ] = (array) get_post_meta( $product_id, '_fcms_woothumbs', true );
		}
	...
	public static function get_setting( $product_id, $setting ) {
		...
		if ( ! isset( $settings[ $setting ] ) ) {
			return apply_filters( 'fcms_woothumbs_get_product_setting', null, $product_id, $setting, $settings );
		}

		return apply_filters( 'fcms_woothumbs_get_product_setting', $settings[ $setting ], $product_id, $setting, $settings );
	}
}
