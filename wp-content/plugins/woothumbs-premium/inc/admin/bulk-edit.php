...
echo '<div class="wrap">';
	...
	echo '<h2 style="margin-bottom: 20px;">'.__('Bulk Edit Variation Images', 'fcms-woothumbs').'</h2>';

	echo '<p>'.sprintf( __( 'From this page you can edit all variation images from one place. Simply enter a comma separated list (no spaces) of image IDs for each variation and click Update. You can find your image IDs in the <a href="%s" target="_blank">Media Library</a>.', 'fcms-woothumbs'), admin_url('upload.php') ).'</p>';
	...
	if ( $variations->have_posts() ) {
			...
			echo '<tbody>';
				...
				$i = 0; while ( $variations->have_posts() ) {
					...
					echo '<tr class="'.($i % 2 == 0 ? 'alternate' : '').'">';
						...
						echo '<td><input class="button-primary saveVariationImages" type="submit" value="'.__('Update', 'fcms-woothumbs').'" data-updating="'.__('Updating...', 'fcms-woothumbs').'" data-updated="'.__('Updated', 'fcms-woothumbs').'" data-update="'.__('Update', 'fcms-woothumbs').'" data-varid="'.get_the_id().'" data-input="#images-'.$i.'" data-error="'.__('Error', 'fcms-woothumbs').'"> <p class="updateMsg"></p></td>';
					echo '</tr>';
    	...
    	echo sprintf('<strong>%s</strong>', __('Sorry, no variations were found.', 'fcms-woothumbs'));
