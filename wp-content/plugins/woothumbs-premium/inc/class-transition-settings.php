...
/**
 * FCMS_WooThumbs_Transition_Settings.
 *
 * Transition any old settings to new ones
 *
 * @class    FCMS_WooThumbs_Transition_Settings
 * @version  1.0.0
...
class FCMS_WooThumbs_Transition_Settings {
	...
	public function transition_redux() {
		...
		foreach ( $redux_settings as $key => $value ) {
			switch ( $key ) {
				...
				case 'slideDuration':
					...
					break;
				case 'largeImageSize':
					$new_settings['display_images_large_image_size'] = $value;
					break;
				case 'iconColour':
		...
		add_option( 'fcms_woothumbs_settings', $new_settings );
	...
	public function transition_meta() {
		$option_name = 'fcms_woothumbs_meta_transitioned';
		$meta_transitioned = get_option( $option_name );

		if ( $meta_transitioned ) {
			return;
		}
		...
		$wpdb->update(
			$wpdb->postmeta,
			array( 'meta_key' => '_fcms_woothumbs' ),
