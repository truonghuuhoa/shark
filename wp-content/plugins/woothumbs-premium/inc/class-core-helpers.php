...
if ( class_exists( 'FCMS_WooThumbs_Core_Helpers' ) ) {
	return;
}

/**
 * FCMS_WooThumbs_Core_Helpers.
 *
 * @class    FCMS_WooThumbs_Core_Helpers
 * @version  1.0.1
 */
class FCMS_WooThumbs_Core_Helpers {
	/**
	...
	/**
	 * Check variations of fcms plugins.
	 *
	...
	public static function get_plugin_name_variations( $plugin ) {
		...
		if ( strpos( $plugin, 'fcms-' ) !== 0 ) {
			return $plugins;
		}

		$plugin_exploded = explode( '/', $plugin );
		$prefix_removed  = str_replace( 'fcms-', '', $plugin_exploded[0] );
