...
class FCMS_WAS_Autoloader {
	...
	private function autoload( $class_name ) {
		...
		$file_name = strtolower( str_replace(
			array( $this->class_prefix, 'FCMS_', '_' ),      // Prefix | Plugin Prefix | Underscores
	...
	public function autoload_versioned( $class_name ) {
		if ( empty( $GLOBALS['fcms_versioned_classes'] ) || empty( $GLOBALS['fcms_versioned_classes'][ $class_name ] ) ) {
			return;
		}

		$versions = $GLOBALS['fcms_versioned_classes'][ $class_name ];
